...
\newcommand{\mybiblatexfile}{paper_references.bib}
...
%% general metadata:
\newcommand{\myauthor}{Oliver Reichmann}  %% also used for PDF metadata (hyperref)
\newcommand{\myauthorwithexistingtitles}{\myauthor{}}  %% including
                                %% university degree already held
                                %% (BSc, MSc, ...)
\newcommand{\mytitle}{Using Augmented Reality to give Instructions for Building LEGO Models}  %% also used for PDF metadata (hyperref)
...
\newcommand{\myinstitute}{Institute of Computer Graphics and Knowledge Visualisation} %% affiliation
\newcommand{\myinstitutehead}{Univ.-Prof. Dr.rer.nat. M.Sc.~Tobias Schreck} %% head of institute
\newcommand{\mysupervisor}{Assoc.Prof. M.Sc.~PhD Ursula Augsdörfer} %% your supervisor
\newcommand{\mycosupervisor}{Dipl.-Ing. Dr.techn.~Andreas Riffnaller-Schiefer}  %% only used with title_Thesis_TU_Graz_-_kazemakase
\newcommand{\myevaluator}{Prof.~Some Genius} %% your evaluator
\newcommand{\myhomestreet}{Jakob-Redtenbacher-Gasse~20} %% your home street (with house number)
...
\newcommand{\myid}{01611723} %% Matrikelnummer
...
% \include{first_draft}
% \include{example-style-chapter}
 \include{ar}             %% this is a suggestion: you have to create this file on demand
